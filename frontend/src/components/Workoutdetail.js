...
function Workoutdetail() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        // Only update the state if the response is successful
        setTitle("");
        setLoad("");
        setReps("");
        notify();
      ...
      <form className="bg-white shadow-md rounded absolute top-[30%] px-32 pt-6 pb-10 mb-4" onSubmit={handleSubmit}>
        <div>
          ...
          <div className="mb-4">
            ...
            <input
              className="shadow appearance-none text-capitalize border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
              id="title"
              type="text"
